-- WinMix Engine Statement Timeout + H2H Direction Constraint
-- Apply this in the Supabase SQL Editor (Dashboard > SQL > New Query)
-- or via the Supabase MCP apply_migration tool if available.
--
-- This migration does two things:
-- 1. Sets statement_timeout = '120s' on the three SECURITY DEFINER engine
--    functions so the Edge Function worker doesn't hit Supabase's default
--    session timeout (8-30s on some tiers) when aggregating 24,720 rows.
-- 2. Adds the missing CHECK (home_team_id <> away_team_id) constraint
--    to winmix_h2h_pairs.

-- 1. Increase statement_timeout on engine RPC functions to 120 seconds.
...
-- 2. Add missing direction constraint on winmix_h2h_pairs.

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'winmix_h2h_pairs_distinct_teams'
      and conrelid = 'public.winmix_h2h_pairs'::regclass
  ) then
