/*
# WinMix Cloud Tier — Séma, RLS, nézetek

## Overview
Creates the database schema for the WinMix cloud tier — an optional, read-only
cross-check layer that mirrors the local pipeline's team ratings. The app is
offline-first; Supabase is additive only. The browser client never writes to
these tables — it only reads `view_team_ratings` to diff against local computations.

## New Tables
1. `winmix_teams` — canonical teams per league, with weight index and source.
2. `winmix_seasons` — season metadata (1:1 with the Season interface).
3. `winmix_matches` — individual match results with generated columns for
   total_goals, btts, and outcome (ensures row-level consistency).
4. `winmix_pipeline_checkpoints` — diagnostic archive for pipeline state
   (server-side only; the client uses sessionStorage).

## View
- `view_team_ratings` — replicates `computeAutoTeamWeights()` from
  `utils/autoWeights.ts` in SQL, partitioned by league. Must match the TS
  output to 2 decimal places for the cross-check diff to pass.

## Security
- RLS enabled on all 4 tables.
- SELECT-only policies for `anon` and `authenticated` (the app uses a
  publishable key, no sign-in).
- No INSERT/UPDATE/DELETE policies — writes are blocked by default.
  Only `service_role` (server-side ingestion) can write, bypassing RLS.
- GRANT SELECT on all tables + view to `anon` and `authenticated`.
- REVOKE write privileges from anon/authenticated.
- View uses `security_invoker = true` so base-table RLS always applies.

## Important Notes
1. The `winmix_` prefix avoids collisions with pre-existing `teams`/`matches` tables.
2. Generated columns (`total_goals`, `btts`, `outcome`) prevent inconsistent rows.
3. The view uses `stddev_pop` (not `stddev_samp`) to match the TS `variance` function.
4. `nullif(stddev, 0)` → `1` reproduces the TS `std || 1` fallback.
5. The PPG prior (1.35) only applies when `played = 0`, not for NULL.
6. A `left join` keeps zero-match teams so they don't skew the mean.
7. Partitioning is per-league — the two leagues are never standardized together.
*/
...
-- Pipeline checkpointok — kizárólag diagnosztikai archívum.
...
-- SELECT-only policies (read-only cloud tier; writes only via service_role)
drop policy if exists winmix_teams_read_all       on public.winmix_teams;
...
-- ============================================================
-- 3. GRANT — az RLS önmagában nem elég
-- ============================================================
...
-- ============================================================
-- 4. VIEW — view_team_ratings (keresztellenőrzés alapja)
-- ============================================================
...
-- A nézet security_invoker opciója biztosítja, hogy az alaptáblák RLS-e érvényesül
